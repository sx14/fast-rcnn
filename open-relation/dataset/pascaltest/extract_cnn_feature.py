...
def prepare_object_boxes_labels(anno_root, anno_list_path, box_save_path, label_save_path):
    ...
    for i in range(0, len(anno_list)):
        anno_file_id = anno_list[i]
        print('prepare processing[%d/%d] %s' % (len(anno_list), (i + 1), anno_file_id))
        anno = org.pascal_anno_2_dict(os.path.join(anno_root, anno_list[i]+'.xml'))
        image_id = anno['filename'].split('.')[0]
...
if __name__ == '__main__':
    pascal_root = data_config.VOC_ROOT
    ...
    for d in datasets:
        label_save_path = os.path.join(pascal_root, 'feature', target, 'prepare', d + '_labels.bin')
        box_save_path = os.path.join(pascal_root, 'feature', target, 'prepare', d + '_boxes.bin')
        fc7_save_root = os.path.join(pascal_root, 'feature', target, 'fc7')
        label_save_root = os.path.join(pascal_root, 'feature', target, 'label', d + '.txt')
        anno_root = os.path.join(pascal_root, 'Annotations')
        anno_list = os.path.join(pascal_root, 'ImageSets', 'Main', d + '.txt')
        img_root = os.path.join(pascal_root, 'JPEGImages')
        if target == 'object':
        ...
        wn2index_path = os.path.join(pascal_root, 'feature', target, 'prepare', 'wn2index.json')
        with open(wn2index_path, 'r') as wn2index_file:
            wn2index = json.load(wn2index_file)
        label2wn_path = os.path.join(pascal_root, 'feature', target, 'prepare', 'label2wn.json')
        ...
        extract_fc7_features(net, boxes, labels, img_root, anno_list, fc7_save_root, label_save_root,
                              wn2index, label2wn)
    small_val_list_path = os.path.join(pascal_root, 'feature', target, 'label', 'val_small.txt')
    val_list_path = os.path.join(pascal_root, 'feature', target, 'label', 'val.txt')
